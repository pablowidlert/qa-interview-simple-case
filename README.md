...
- You can also run the tests with the following command `npm run test:e2e` or `npm run test:e2e:headed`. Note: You do not need to start the app locally as this is handled in the playwright.config.ts

- If you want to run the app locally using one of the scripts defined in `package.json`. You can try with `npm run dev` and you should see the app running locally and a link to open it in your preferred browser:
